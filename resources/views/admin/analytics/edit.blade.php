    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.analytics.update", [$analytic->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bvc') ? 'is-invalid' : '' }}" type="text" name="bvc" id="bvc" value="{{ old('bvc', $analytic->bvc) }}">
                @if($errors->has('bvc'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('game') ? 'is-invalid' : '' }}" name="game_id" id="game_id" required>
                    @foreach($games as $id => $entry)
                        <option value="{{ $id }}" {{ (old('game_id') ? old('game_id') : $analytic->game->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('game'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('entry') ? 'is-invalid' : '' }}" type="text" name="entry" id="entry" value="{{ old('entry', $analytic->entry) }}">
                @if($errors->has('entry'))
